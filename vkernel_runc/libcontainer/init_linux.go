...
func finalizeNamespace(config *initConfig, vkn *vkernel.VKernel) error {
	...
	// leave the responsibility of applying capabilities to vkernel module
	if vkn != nil {
		params := vkernel.GenerateCapabilityParams(w.effective, w.permitted, w.inheritable, w.bounding, w.ambient)
		params = fmt.Sprintf("name=%s %s", vkn.Name, params)
		if err := vkn.InitVKernel(params); err != nil {
			return errors.Wrap(err, "init vkernel")
		}
	}
	return nil
}
