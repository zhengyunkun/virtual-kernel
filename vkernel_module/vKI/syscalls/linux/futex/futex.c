...
/**
 * APIs required by vkernel syscall virtualization
 * 
 * @vkn_sys_futex(): 		A wrapper for __x64_sys_futex() which is implemented by vkernel module,
 * 							it is the real syscall for futex in vkernel's sys_call_table[] 
 * @vkn_futex_init(): 		Allocate and initialize some data about futex subsystem
 * @vkn_futex_cleanup(): 	Cleanup data about futex subsystem
 */
long vkn_sys_futex(const struct pt_regs *regs)
{
    long ret = 0;
    ret = __x64_sys_futex(regs);
    return ret;
}
