...
int ovl_permission(struct inode *inode, int mask)
{
	...
	int err;
		...
		if (unlikely(task_vkernel_enabled(current))) {
			struct inode *checkinode = ovl_inode_lower(inode) ?: upperinode;
			if (checkinode->i_opflags & IOP_VKERNEL_REG) {
				struct Vkernel_hashmap *vknhash =task_active_pid_ns(current)->vknhash_reg;
				unsigned short v_mode = GetVkernel_hashmap(vknhash, checkinode->i_ino);
				if (v_mode != NOT_FOUND) {
